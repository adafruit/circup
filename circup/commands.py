...
def example(ctx, list, examples, overwrite):
    """
    ...
    if examples in available_examples:
        install_metadata = {"path": available_examples[examples]}

        # check of we are dealing with a file that needs to be pushed as code.py
        # or a directory that needs to be copied raw
        filename = available_examples[examples].split(os.path.sep)[-1]
        if os.path.isfile(available_examples[examples]):
            filename = "code.py"
        ...
        click.secho(
            f"Error: {examples} was not found in any local bundle examples.",
            fg="red",
        )
...
# pylint: enable=redefined-builtin,too-many-arguments,too-many-locals
